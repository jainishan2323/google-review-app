...
- **Dashboard is pinned to a hardcoded business.** `apps/web` pages read Postgres filtered by `DEV_BUSINESS_ID` (a seeded business). There is no per-user business resolution on `main`.
...
- **`apps/web` `/api/generate-reply` has auth disabled** (TODO to re-enable once Google API is approved and `session.userId` reliably maps to the Prisma `User.id`).
